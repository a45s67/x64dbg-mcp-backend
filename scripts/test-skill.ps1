...
Assert-Skill ($frontmatter.Groups[1].Value -match '(?m)^  version: "0\.6\.0"$') 'independent skill version is missing'
Assert-Skill ($frontmatter.Groups[1].Value -match '(?m)^  minimum-backend-version: "0\.6\.0"$') 'minimum backend version is missing'
