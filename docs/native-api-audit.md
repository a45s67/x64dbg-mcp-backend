...
re-formatted numeric address. `debuggee.launch` and `debuggee.launch_dll`
contain only canonical existing paths in fixed quoted `scriptcmd init`
positions. Bounded PE headers establish exact EXE/DLL kind and backend machine
before mutation. EXE arguments are Windows-quoted by the plugin and committed
at the initial actionable pause with `DBGFUNCTIONS::SetCmdline`, never inserted
as caller command syntax. DLL launch has no argv and respects x64dbg's fixed
...
| typed breakpoint enable/disable (ADR 0039) | fixed explicit-identity `bpe`/`bpd`, `bphwe`/`bphwd`, `bpme`/`bpmd`, or `EnableExceptionBPX`/`DisableExceptionBPX`; private command fence; `GetBridgeBp`, `DbgGetBpList`, `MemBpSize` | paused; hardware enable requires actionable pause | one exact closed selector; no bulk form; before/after configuration invariant; managed conditional/exception identity; hardware slot is nullable observation and may change; four-slot preflight list released with `BridgeFree` |
...
| `debuggee.launch_dll` | bounded PE-header read, `DbgCmdExec` (`scriptcmd init`), `Script::Module::GetList` | absent | exact architecture-matched DLL and existing x64dbg `loaddll.exe`; path below fixed mapping bound; confirms initial generated loader module while target is absent; no argv/export/hidden resume; module list released with `BridgeFree`; post-submit failure is outcome-unknown |
